...
public class PoisonTheory extends EnchantTemplate {
	
	public PoisonTheory(EnchantmentTarget enTarget, String name, int maxLevel) {
	...
	public void apply(TurboPlayer killer, LivingEntity victim, int level) {
		...
		PotionEffect pe = new PotionEffect(PotionEffectType.POISON, startDuration + durationPerStep * (level / levelsByStep), amplifier, true);
		pe.apply(victim);
	}
