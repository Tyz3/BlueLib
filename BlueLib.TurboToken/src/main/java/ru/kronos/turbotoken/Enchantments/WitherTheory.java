...
public class WitherTheory extends EnchantTemplate {
	
	public WitherTheory(EnchantmentTarget enTarget, String name, int maxLevel) {
	...
	public void apply(TurboPlayer killer, LivingEntity victim, int level) {
		...
		PotionEffect pe = new PotionEffect(PotionEffectType.WITHER, startDuration + durationPerStep * (level / levelsByStep), amplifier, true);
		pe.apply(victim);
	}
