...
public class BlindnessTheory extends EnchantTemplate {
	
	public BlindnessTheory(EnchantmentTarget enTarget, String name, int maxLevel) {
	...
	public void reload(ConfigurationSection c) {
		...
		durationPerStep = c.getInt("durationPerStep", 40);
	...
	public void apply(TurboPlayer killer, LivingEntity victim, int level) {
		...
		PotionEffect pe = new PotionEffect(PotionEffectType.BLINDNESS, startDuration + durationPerStep * (level / levelsByStep), amplifier, true);
		pe.apply(victim);
	}
